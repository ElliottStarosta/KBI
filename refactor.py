...
def create_weekly_dataframes(df, target_month, target_year):
    ...
    if not weeks:
        print("No valid weeks found for the target month")
...
def main():
    ...
    if len(file_paths) < 3:
        print(f"Error: Found only {len(file_paths)} CSV files in '{folder_path}'. At least three CSV files are required.")
        return
    ...
    if len(file_paths) > 3:
        ...
        print(f"Using the three most recent CSV files: {[os.path.basename(f) for f in file_paths]}")
    
    # Find the middle month
    print("Determining the middle month from the provided files...")
    ...
    print(f"Middle month determined as {target_month_name} {target_year}")
    ...
    # Create weekly dataframes
    print("Identifying complete weeks and creating sheet data...")
